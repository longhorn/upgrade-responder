...
## Overview
Upgrade Responder provides a way for the applications to know if there is a new version available. 
It will also keep a record of how many requests have been sent to the Upgrade Responder server per hour, to estimate how many instances of applications are running there.
...
1. Once the server received a request, it will:
   ...
   1. Store the following information into an InfluxDB table:
      * Application Version
      * Country and City of the request originated from
      * X-Request-ID which is a random string per request that is generated by the client.
      * Other information that's can be helpful to identify the upgradability, e.g. Kubernetes version.
...
| `--port` | `8000` | Specify the port number. By default port `8314` is used |
...
curl -X POST http://<SERVER-IP>:8314/v1/checkupgrade \
     ...
     -H "X-Request-Id: 8ebbb83a-a224-49f9-aa73-8563d1da89b7" \
     -H "X-Forwarded-For: <YOUR-IP-ADDRESS>" \
...
1. If you specify `--influxdb-continuous-query-period` to be different than `1h`, 
   you need to change the time in GROUP BY clause in Grafana dashboard queries to match `--influxdb-continuous-query-period`
...
By default, your application should send one `POST` request every hour.
The request must meet the following requirements:
1. The request's header must contains `X-Forwarded-For` key with the value that contains the IP of the host where your application instance is running on. We use the IP to look up the country code and discard it immediately. We don't store the IP.
1. The request's header must contains `X-Request-Id` key with a random value
1. The request's body should be in the format:
   ```json
   {
    "appVersion": "v0.8.1",
    "extraInfo": {
   ...
   If you add extra fields and want to display statical information for those fields, you have to do extra steps. 
...
If you want to keep track of the number of your application instances by each Kubernetes version, you may want to include Kubernetes version into `extraInfo` in the request's body sent to Upgrade Responder server.
...
1. Create a continuous query in the InfluxDB to periodically group data by Kubernetes version
   ```
   CREATE CONTINUOUS QUERY "cq_by_kubernetes_version_down_sampling" ON "<application-name>_upgrade_responder" BEGIN SELECT count("x_request_id") as total INTO "by_kubernetes_version_down_sampling" FROM "upgrade_request" GROUP BY time(1h),"kubernetes_version" END
   ```
   where:
...
### InfluxDB
We currently only support InfluxDB version 1.8 and before
...
Your application will have to send request to Upgrade Responder server every hour.

Normally, `--query-period` should be decided at the beginning and should not be changed after it is set the first time. 
Changing the value may cause temporary data miss-match in InfluxDB. If you have to change `--query-period`, please follow the steps:
...
1. Change the time in GROUP BY clause in Grafana dashboard queries to match `--influxdb-continuous-query-period`
1. Modify your application to send requests to Upgrade Responder server every `--influxdb-continuous-query-period` interval
